...
export class GeminiEnrichmentAgent extends BaseGeminiAgent {
  ...
  protected buildPrompt(context: Record<string, unknown>): string {
    return [
      ...
      "",
      "--- CORE CONSTRAINTS (HALLUCINATION PREVENTION) ---",
      "- Output ONLY what you can infer with high confidence from the domain and name.",
      "- If you cannot determine a field with >60% confidence, output null for that field.",
      "- Never invent a specific employee count — use ranges only (1-50, 51-200, 201-1000, 1000+).",
      "- Never invent a person's specific title — use role category only (Engineering, Sales, Marketing).",
      "- Do not use company names you recognize from training data unless the domain exactly matches.",
      "",
      "--- CONTEXT ---",
